...
class RSIBullishBase(RSISignalBase):
    ...
    def calculate_ohlc_rsi_roc(self, data: pd.DataFrame, price_suffix: str = '') -> Tuple[pd.Series, pd.Series]:
        ...
        # For BTC quotes of non-bitcoin assets, we need to use the BTC-adjusted OHLC columns
        # These are created by the data loader as {asset_id}_btc_open, etc.
        if price_suffix == '_btc':
            # Extract asset_id from params
            asset_id = self.params.get('asset_id', '')
            if asset_id:
                # Use the BTC-adjusted OHLC columns
                open_col = f'{asset_id}_btc_open'
                high_col = f'{asset_id}_btc_high'
                low_col = f'{asset_id}_btc_low'
                close_col = f'{asset_id}_btc'
