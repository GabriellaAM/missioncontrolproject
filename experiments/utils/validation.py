...
def permutation_test(original_returns: np.ndarray, 
                    strategy_returns: np.ndarray,
                    n_permutations: int = 1000,
                    metric: str = 'sharpe',
                    random_seed: Optional[int] = 42,
                    ohlc_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    """
    Perform permutation test to validate strategy performance using sophisticated OHLC permutation.
    
    This function uses block-based permutation that preserves market microstructure while
    destroying temporal patterns. If OHLC data is provided, it performs proper financial
    time series permutation. Otherwise, falls back to simple shuffling.
    
    Args:
        original_returns: Original asset returns
        strategy_returns: Strategy returns
        n_permutations: Number of permutations
        metric: 'sharpe' or 'profit_factor'
        random_seed: Random seed for reproducibility
        ohlc_data: Optional DataFrame with OHLC columns ['open', 'high', 'low', 'close']
                  If provided, enables sophisticated OHLC-based permutation
    ...
    if metric == 'sharpe':
        original_metric = calculate_sharpe_ratio(strategy_returns)
    elif metric == 'profit_factor':
        signals = np.sign(strategy_returns)
        original_metric = calculate_profit_factor(original_returns, signals)
    else:
    ...
    for i in range(n_permutations):
        if ohlc_data is not None:
            # Use sophisticated OHLC permutation that preserves microstructure
            permuted_ohlc = create_ohlc_permutation(
                ohlc_data, 
                start_index=0,
                seed=random_seed + i if random_seed else None
            )
            # Calculate returns from permuted close prices
            permuted_returns = np.log(permuted_ohlc['close'] / permuted_ohlc['close'].shift(1)).dropna().values
        else:
            # Fallback to simple permutation (preserves for backward compatibility)
            permuted_returns = np.random.permutation(original_returns)
        
        if metric == 'sharpe':
            permuted_metric = calculate_sharpe_ratio(permuted_returns)
        elif metric == 'profit_factor':
            signals = np.sign(strategy_returns)
            permuted_metric = calculate_profit_factor(permuted_returns, signals)
        
        # Only include finite values
        if np.isfinite(permuted_metric):
            permuted_metrics.append(permuted_metric)
    
    # Calculate p-value
    permuted_metrics = np.array(permuted_metrics)
    p_value = np.mean(permuted_metrics >= original_metric) if len(permuted_metrics) > 0 else 1.0
    ...
    return {
        ...
        'n_valid_permutations': len(permuted_metrics),
        'permutation_method': 'ohlc_based' if ohlc_data is not None else 'simple_shuffle'
    }


def walk_forward_validation(X: pd.DataFrame, 
                           y: pd.Series,
                           model: Any,
    ...
    for i in range(initial_train_size, n_samples - test_size, test_size):
        ...
        train_end = i
        test_start = i
        test_end = min(i + test_size, n_samples)
        ...
        if i == initial_train_size or (i - initial_train_size) % retrain_frequency == 0:
            try:
                model.fit(X_train, y_train)
            except Exception as e:
                print(f"Warning: Model fitting failed at fold {len(results['fold_metrics'])}: {e}")
                continue
        ...
        try:
            ...
            print(f"Warning: Prediction failed at fold {len(results['fold_metrics'])}: {e}")
            continue
        ...
        fold_returns = y_test.values
        results['fold_returns'].append(fold_returns)
        
        if len(fold_returns) > 0:
            fold_sharpe = calculate_sharpe_ratio(fold_returns)
            results['fold_metrics'].append({
                'fold': len(results['fold_metrics']),
                'sharpe': fold_sharpe,
    ...
    if len(results['actuals']) > 0:
        all_returns = np.array(results['actuals'])
        predictions = np.array(results['predictions'])
        
        results['overall_sharpe'] = calculate_sharpe_ratio(all_returns)
        results['overall_profit_factor'] = calculate_profit_factor(
            all_returns, 
            np.sign(predictions) if len(predictions) > 0 else None
        )
        results['n_folds'] = len(results['fold_metrics'])
        results['total_samples'] = len(all_returns)
    else:
        results['overall_sharpe'] = 0.0
        results['overall_profit_factor'] = 0.0
        results['n_folds'] = 0
...
def permuted_walk_forward_test(X: pd.DataFrame,
                              y: pd.Series,
                              model: Any,
    ...
    """
    Perform permuted walk-forward validation to test strategy robustness.
    
    Args:
        X: Feature matrix
        y: Target variable
        model: Model instance with fit and predict methods
        wf_params: Parameters for walk_forward_validation
    ...
    print("Running original walk-forward validation...")
    original_results = walk_forward_validation(X, y, model, **wf_params)
    original_sharpe = original_results['overall_sharpe']
    
    # Permuted walk-forward tests
    print(f"Running {n_permutations} permuted walk-forward validations...")
    permuted_sharpes = []
    ...
    for i in range(n_permutations):
        ...
        # Shuffle y while maintaining structure
        ...
        try:
            perm_results = walk_forward_validation(X, y_permuted, model, **wf_params)
            permuted_sharpes.append(perm_results['overall_sharpe'])
        except Exception as e:
    ...
    return {
        ...
        'p_value': p_value,
        'passes_test': p_value < p_value_threshold,
        'threshold': p_value_threshold,
        'percentile': (1 - p_value) * 100,
        'n_permutations': n_permutations,
        'original_n_folds': original_results['n_folds'],
        'original_total_samples': original_results['total_samples']
    }
