...
def debug_atr_calculo(
    ...
    """
    Debug function to compare ATR trailing stop calculation with TradingView.
    Prints intermediate values for each bar so you can compare.
    """
    df = ler_ohlc_parquet(coingecko_id)
    if df is None:
        print(f"Data not found for {coingecko_id}")
        return

    # Calculate ATR on full history
    df = df.copy()
    df['atr'] = calcular_atr(df, atr_period)

    # Filter from ONE DAY BEFORE entry date
    ...
    print(f"\n{'='*80}")
    print(f"DEBUG ATR Trailing Stop: {coingecko_id}")
    print(f"Entry: {data_entrada} | Side: {side} | ATR({atr_period}, {multiplier})")
    print(f"{'='*80}")
    print(f"{'Bar':<4} {'Date':<12} {'Close':>12} {'ATR':>12} {'Candidate':>12} {'Trail':>12}")
    print(f"{'-'*80}")

    for i in range(min(num_bars, len(df_filtered))):
        ...
        if pd.isna(atr):
            print(f"{i:<4} {date_str:<12} {close:>12.2f} {'NA':>12} {'':>12} {'':>12}")
            continue

        if trail is None:
            # Setup bar (day before entry)
            if is_long:
                trail = close - (multiplier * atr)
            else:
                trail = close + (multiplier * atr)
            print(f"{i:<4} {date_str:<12} {close:>12.2f} {atr:>12.2f} {'(setup)':>12} {trail:>12.2f}")
        else:
            ...
            if pd.isna(prev_atr):
                print(f"{i:<4} {date_str:<12} {close:>12.2f} {atr:>12.2f} {'NA prev':>12} {trail:>12.2f}")
                continue

            if is_long:
                candidate = prev_close - (multiplier * prev_atr)
                new_trail = max(trail, candidate)
            else:
                candidate = prev_close + (multiplier * prev_atr)
                new_trail = min(trail, candidate)

            breached = (is_long and close <= new_trail) or (not is_long and close >= new_trail)
            status = " BREACH!" if breached else ""

            print(f"{i:<4} {date_str:<12} {close:>12.2f} {atr:>12.2f} {candidate:>12.2f} {new_trail:>12.2f}{status}")
            trail = new_trail
    ...
    print(f"{'='*80}")
    print(f"Compare these values with TradingView to find the discrepancy.\n")
