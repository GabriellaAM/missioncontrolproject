...
class TrendAnalyzer:
    ...
    def create_portfolio(self, portfolio_name, btc_trend_gating=None, usd_conditions=None, btc_conditions=None, 
        ...
        """
        Create a new portfolio with specified trend and RSI conditions.
        ...
        self.logger.info(f"Portfolio '{portfolio_name}' created with criteria: BTC gating={btc_trend_gating}, "
                        ...
                        f"RSI USD={rsi_conditions_usd}, RSI BTC={rsi_conditions_btc}, BTC only={btc_only}")
    ...
    def backtest_portfolio(self, portfolio_name, start_date, end_date, initial_capital=1000, alt_cost=0.005, btc_cost=0.001):
        ...
        if btc_only:
            ...
            for i, date in enumerate(dates):
                ...
                rsi_usd_signal = latest_btc.get('RSI_Signal_close', 0) == 1 if rsi_conditions_usd else True
                
                # BTC Decision Funnel: Trend conditions + RSI vs USD
                current_signal = usd_condition and btc_condition and rsi_usd_signal and btc_gate_passed
                if i > 1:
            ...
            for i, date in enumerate(dates):
                ...
                for asset_id in self.asset_ids:
                    ...
                    rsi_usd_signal = latest.get('RSI_Signal_close', 0) == 1 if rsi_conditions_usd else True
                    rsi_btc_signal = latest.get(f'RSI_Signal_{asset_id}_btc', 0) == 1 if rsi_conditions_btc else True
                    
                    # Calculate combined signal
                    combined_signal = 0
                    if usd_condition and btc_condition and btc_gate_passed:
                        if rsi_conditions_usd:
                            combined_signal += latest.get('RSI_Signal_close', 0)
                        if rsi_conditions_btc:
                            combined_signal += latest.get(f'RSI_Signal_{asset_id}_btc', 0)
                        # If no RSI conditions, default to positive signal
                        if not rsi_conditions_usd and not rsi_conditions_btc:
                            combined_signal = 1
